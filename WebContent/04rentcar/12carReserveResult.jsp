...
<body>
	...
	<jsp:useBean id="rbean" class = "_04rentcar.CarReserveBean">
	...
	if (id == null) {
		...
		<script>
			...
			location.href="01carMain.jsp?center=05memberLogin.jsp";
		</script>	
	<%	
	}
	%>
	...
	d2 = sdf.parse(sdf.format(d2)); //format 메서드는 SimpleDateFormat과 같은 역할(yyyy-MM-dd)
	
	//날짜 비교 메서드 사용
	int compare = d1.compareTo(d2);
	//d1은 예약 날짜 / d2는 현재 날짜
	//예약하려는 날짜가 현재 날짜보다 이전이라면 -1
	//예약하려는 날짜가 현재 날짜와 같다면 0
	//예약하려는 날짜가 현재 날짜보다 이후라면 1을 리턴
	System.out.println(compare);
	
	if (compare < 0) { //현재 날짜보다 이전 날짜를 선택
		%>	
		<script>
